...
class MostOrangeElementDetector:
    @classmethod
    def INPUT_TYPES(s):
        return {
            "required": {
    ...
    RETURN_TYPES = (
        ...
        "INT", "INT", "INT",  # Mid RGB (purest orange)
    ...
    FUNCTION = "detect_orange"

    CATEGORY = "Color Detection"

    def detect_orange(self, image, threshold):
        ...
        # Calculate "orangeness" score (maximize red, medium green, minimize blue)
        # Formula: high red + moderate green - blue
        orangeness = red_channel + green_channel / 2 - blue_channel
        orangeness_norm = (orangeness - orangeness.min()) / (orangeness.max() - orangeness.min())

        # Create mask for purest orange
        mid_mask = orangeness_norm >= 0.99
        if mid_mask.sum() == 0:
            mid_mask = orangeness_norm >= orangeness_norm.max()

        # Calculate mid RGB values (purest orange)
